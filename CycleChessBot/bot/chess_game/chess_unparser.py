...
from chess_game.board import PieceTypeAndColor, BoardRepresentation
...
class ChessUnparser():
    ...
    def move(self, move_str: str) -> Move:
        move = move_str.split('.')
        return Move(self.position(move[0]), self.position(move[1]), PieceType(int(move[2])) if len(move) > 2 else None)

    def color(self, color_str: bytearray) -> Color:
        color_int = int(color_str.decode('utf-8').split(':')[1])
    ...
    def possible_positions(self, possible_positions_str: bytearray) -> list[Position]:
        possible_positions = possible_positions_str.decode('utf-8').split(':')[1].split(';')
    ...
    def board_representation(self, board_representation_str: bytearray) -> BoardRepresentation:
        ...
        for i in range(8):
            ...
            for j in range(8):
                ...
                board_repr[i].append(PieceTypeAndColor(PieceType(piece_type_int), Color(color_int)))
        return board_repr
