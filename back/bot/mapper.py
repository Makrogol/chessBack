...
# -1 +1
def is_south_west_over_board(
    position_first: Position, position_second: Position
) -> bool:
    return position_second == make_offset(position_first, -1, 1)


# -1 -1
def is_south_east_over_board(
    position_first: Position, position_second: Position
) -> bool:
    return position_second == make_offset(position_first, -1, -1)
...
def get_knigh_over_board_move(
    ...
    raise Exception(
        f"Invalid knight over board move, position_first={position_first}, position_second={position_second}"
    )
...
class Mapping:
    ...
    * the value is the plane's index, or an array of plane indices (for distance)
    ...
    def get_index(
        self, piece_type: PieceType, direction: Enum, distance: int = 1
    ) -> int:
    ...
    @staticmethod
    def get_underpromotion_move(
        piece_type: PieceType, position_first: Position, position_second: Position
    ) -> Tuple[UnderPromotion, int]:
        ...
        # Шаг по прямой
        if (
            make_offset(position_first, 1, 0) == position_second
            or make_offset(position_first, -1, 0) == position_second
        ):
            direction = 1
        # Шаг для пешки по диагонали вправо
        elif (
            make_offset(position_first, 1, -1) == position_second
            or make_offset(position_first, -1, 1) == position_second
        ):
            direction = 2
        # Шаг для пешки по диагонали влево
            ...
            raise Exception(
                f"Invalid underpromotion move, position_first={position_first}, position_second={position_second}"
            )
    ...
    @staticmethod
    def get_queenlike_move(
        position_first: Position, position_second: Position
    ) -> Tuple[QueenDirection, int]:
        from_square = position_first.i * 8 + position_first.j
        to_square = position_second.i * 8 + position_second.j
        diff = from_square - to_square
        if diff % 8 == 0:
            ...
            raise Exception(
                f"Invalid queen-like move, position_first={position_first}, position_second={position_second}"
            )
