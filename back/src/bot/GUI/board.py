...
class Board:
        ...
        for i in range(len(Board.boardRect)):
            for j in range(len(Board.boardRect[i])):
                if self.is_selected(i, j):
                    ...
                    Board.isEven(i)
                    and Board.isEven(j)
                    or not Board.isEven(i)
                    and not Board.isEven(j)
                ):
                    tile = Board.wtile
                else:
                    tile = Board.btile
    ...
    def is_selected(self, i: int, j: int) -> bool:
        ...
        return (
            ...
            and self.selected_position.i == i
            and self.selected_position.j == j
        )
    ...
    def createPiece(self, color: Color, piece_type: PieceType, position: Position):
        piece = PieceImage(color, piece_type, self.DISPLAYSURF, self.square_size)
        # convert square to pixel position
        position = Position(
            self.square_size * position.i, self.square_size * position.j
        )
