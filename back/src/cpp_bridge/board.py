...
from .color import Color, get_another_color
from .piece_type import PieceType
from .position import Position
from .piece_type_and_color import PieceTypeAndColor
...
def get_piece_type_and_color_from_fen(fen: str) -> PieceTypeAndColor:
    piece_type = get_type_from_fen(fen)
    if fen.isupper():
        return PieceTypeAndColor(piece_type, Color.WHITE)
    return PieceTypeAndColor(piece_type, Color.BLACK)
...
class Board:
    ...
    def create_from_fen(self, fen: str) -> None:
        ...
        if len(fen_elements) == 6:
            self.castling_rights = [
                "K" in fen_elements[iter],
                "Q" in fen_elements[iter],
                "k" in fen_elements[iter],
                "q" in fen_elements[iter],
            ]
            iter += 1
        ...
        self.passant_position = (
            Position(int(fen_elements[iter][0]), int(fen_elements[iter][2]))
            if fen_elements[iter] != "-"
            else None
        )
        iter += 1
        ...
        for i in range(8):
            ...
            while k < len(board_str[i]):
                if board_str[i][k].isdigit():
                    ...
                    piece = Piece(
                        type_and_color.piece_type, type_and_color.color, Position(i, j)
                    )
    ...
    def get_pieces_by_type_and_color(
            ...
            for cell in line:
                if cell.has_piece():
                    if (
                        ...
                        and cell.piece.piece_type == piece_type
                    ):
                        pieces.append(cell.piece)
        return pieces

    def move_piece(self, position_first: Position, position_second: Position) -> bool:
        self.field[position_second.i][position_second.j].set_piece(
            self.field[position_first.i][position_first.j].piece
        )
